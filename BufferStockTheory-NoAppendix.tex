...
where $0<\underline{\tShkEmp} \leq 1 \leq \bar{\tShkEmp}<\infty$.\footnote{\cite{rabaultBorrowing} and~\cite{lsIncFluct} analyze cases where the shock processes have unbounded support.}  Call the cumulative
...
\begin{equation}\begin{gathered}\begin{aligned}\hypertarget{WRPF}{}
...
\begin{equation} \label{eq:lowerltupper}
  \begin{split}
    \underline{\cFunc}(\mRat) < & \usual{\cFunc}(\mRat)  < \bar{\cFunc}(\mRat)  \\
    1 < & \usual{\cFunc}(\mRat)/\underline{\cFunc}(\mRat)  < \bar{\cFunc}(\mRat)/\underline{\cFunc}(\mRat). 
  \end{split}
\end{equation}
But
\begin{align*}%  \label{eq:limitlowerupper}
  ...
  & = 1,
\end{align*}
so as $\mRat \uparrow \infty, \usual{\cFunc}(\mRat)/\underline{\cFunc}(\mRat)
...
In a Harmenberg-invariant economy (and therefore also any Szeidl-invariant economy), a similar proposition that is testable without a panel (that is, using only cross-section data) is a direct implication of the fact that a constant proportion of aggregate permanent income is accounted for by the successive sets of consumers with any particular $\mRat$.  This fact is essentially Harmenberg's definition of the density of the permanent-income-invariant distribution of a variable (say, $\mRat$).  Call $\cLevBF_{t}(\mRat)$ the total amount of consumption at date $t$ by persons with market resources $\mRat$, and note that in the invariant economy it is given by the converged consumption function $\cFunc(\mRat)$ multiplied by the amount of permanent income accruing to such, people $\pRat_{t}(\mRat)\PLev_{t}$.  But Harmenberg's proof shows that $\pRat(\mRat)$ is invariant,\footnote{In his notation, this is $\tilde{\psi}^{\mRat}$.} and we know from above that $\PLev_{t+1} = \PGro \PLev_{t}$.  So 
\begin{equation*}
  \begin{split}
...
Harmenberg has a brief discussion that demonstrates that the covariance between the consumption ratio $\cRat$ and the permanent income level $\pLevBF$ does not shrink to zero over time; so covariances are another potential target of measurement for construction of microfoundations.  Here we draw out some of those implications.
...
\begin{equation}\label{eq:atp2vsatp1}
  \begin{split}
    ...
    \\  \CLevBF_{t+2} & = \Mean[\cRat_{t+2}\pLevBF_{t+2}] = \bar{\cRat}_{t+2}\PGro^{2} + \cv_{t+2}(\cRat_{t+2}, \pLevBF_{t+2})
  \end{split}
\end{equation}
...
The more interesting case is when the economy is Harmenberg- but not Szeidl-invariant.  In that case, if the $\cv$ and the $\Mean[\cRat]$ terms have constant growth factors $\Grw_{\cov}$ and $\Grw_{\bar{\cRat}}$,\footnote{This conditional `if' is not something that Harmenberg proves (or that is proven elsewhere); it is a hypothetical that is useful for the subsequent general argument.} an equation corresponding to \eqref{eq:atp2vsatp1} will hold in $t+n$:
\begin{equation} \label{eq:aRatGrovsCovGronm1}
  \begin{split}
    (\overbrace{\Grw_{\bar{\cRat}}^{n}\bar{\cRat}_{t}}^{\bar{\cRat}_{t+n}}-\Grw^{n-1}_{\cRat}\bar{\cRat}_{t})\PGro^{n} & = (\Grw_{\cv}^{n}-\PGro \Grw_{\cv}^{n-1}) \cv_{t} 
    \\ (\Grw_{\bar{\cRat}}\PGro)^{n-1} (\Grw_{\bar{\cRat}}-1)\bar{\cRat}_{t}\PGro & = \Grw_{\cv}^{n-1}(\Grw_{\cv}-\PGro ) \cv_{t}
  \end{split}
\end{equation}
so for the LHS and RHS to grow at the same rates we need
\begin{equation}\begin{gathered}\begin{aligned}
  \Grw_{\cov}  & = \Grw_{\bar{\cRat}}\PGro \label{eq:aRatGrovscovGro}. 
\end{aligned}\end{gathered}\end{equation}
This is intuitive.  In the Szeidl-invariant economy, this just reproduces our result above that the covariance exhibits balanced growth because $\Grw_{\bar{\cRat}}=1$.  The revised result just says that in the Harmenberg case where the mean value $\bar{\cRat}$ of the consumption ratio $\cRat$ can grow, the covariance must rise in proportion to any ongoing expansion of $\bar{\cRat}$.
\begin{comment}
  \begin{align*}
    \overbrace{\Grw_{\bar{\cRat}}^{n} \bar{\cRat}_{t}}^{\bar{\cRat}_{t+n}}(\Grw_{\bar{\cRat}} - 1)\PGro^{n} & =(\Grw_{\cv} - \PGro) \overbrace{\cv_{t}\Grw_{\cv}^{n}}^{\cv_{t+n}}
    \\                                                                                                 
    (\Grw_{\bar{\cRat}}\PGro/\Grw_{\cv})^{n} \bar{\cRat}_{t}(\Grw_{\bar{\cRat}} - 1) & =(\Grw_{\cv} - \PGro) \cv_{t}
  \end{align*}
  and a corresponding equation will hold in $t+1$:
  \begin{align*}
    \\  \overbrace{\Grw_{\bar{\cRat}} \bar{\cRat}_{t}}^{\bar{\cRat}_{t+1}}(\Grw_{\bar{\cRat}} - 1)\PGro & =(\Grw_{\cv} - \PGro) \overbrace{\cv_{t}\Grw_{\cv}}^{\cv_{t+1}}
  \end{align*}
  \begin{align*}
    (\Grw_{\bar{\cRat}} - 1)\bar{\cRat}_{t}(\Grw_{\bar{\cRat}} - 1)\PGro & =(\Grw_{\cv} - \PGro) \cv_{t}(\Grw_{\cv} - 1)
  \end{align*}
  ...
  \end{align*}
  \begin{equation}\begin{gathered}\begin{aligned}
    % \Grw_{\bar{\cRat}} - 1 & = \Grw_{\cv} - \PGro \\
    % \Grw_{\bar{\cRat}} + \pGro & = \Grw_{\cv}  \\
    \Grw_{\cv} & = \Grw_{\bar{\cRat}} + (\PGro - 1) \label{eq:aRatGroVscov}
  \end{aligned}\end{gathered}\end{equation}
  \begin{align*}
...
\begin{comment}
  ...
  \begin{equation}
    \begin{split}\label{eq:covProblem} 
      ...
      \bar{\cRat}\PGro +\HarmWide{\cv}(\cRat_{t+1},\pLevBF_{t+1})   & = \PGro \left( \bar{\cRat}  +  \HarmWide{\cv}(\cRat_{t},\pLevBF_{t})\right) 
